...
public:
	...
	void BFS(WeightedGraphVertex<V, E> *v)
    {
        ...
        queue->addFromTail(v);
        while (!queue->isEmpty()){
            WeightedGraphVertex<V, E> *cur = queue->removeFromHead()->getData();
            visited->addFromTail(cur);
            std::cout << cur->getData() << " ";
            for (int i=0;(*cur)[i];i++){
                WeightedGraphVertex<V, E> *next = (*cur)[i]->getData()->getAnotherEnd(cur);
                if (!visited->exist(next) && !queue->exist(next))
                    queue->addFromTail(next);
            }
        }
    }
	void DFS(WeightedGraphVertex<V, E> *v)
	{
	    ...
	    while(cur != NULL){
            if(visited->exist(cur->getData()) == NULL){
                cout << cur << " ";
                visited->addFromTail(cur->getData());
            }
            ListNode<WeightedGraphEdge<V, E> *> *e = (*(cur->getData()))[0];
            while(e != NULL){
                WeightedGraphVertex<V, E> *v = e->getData()->getAnotherEnd(cur->getData());
                if(visited->exist(v) == NULL)
                    l->addAfter(cur, v);
                e = e->getNext();
            }
            cur = cur->getNext();
